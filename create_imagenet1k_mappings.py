"""
Create correct index-based pseudo-label mappings for ImageNet-1K.
...
def create_mappings(
    paths_file="results/dinov2_large_imagenet1k_with_paths/image_paths.json",
    pseudo_labels_dir="results/imagenet1k_pseudolabels/pseudo_labels",
    predictions_file="results/imagenet1k_pseudolabels/predictions.npz"
):
    """Create path-based pseudo-label mappings"""
    
    print("="*80)
    print("CREATING CORRECT PATH-BASED PSEUDO-LABEL MAPPING FOR IMAGENET-1K")
    print("="*80)
    
    # Load the image paths (from feature extraction)
    ...
    class_names = path_data['class_names']
    ...
    for idx, path in enumerate(train_paths):
        ...
        full_mapping["train"][path] = {
            ...
            "pseudo_label_name": class_names[pseudo_label],
    ...
    with open(train_csv, 'w') as f:
        ...
        for path in train_paths:
            ...
            f.write(f'"{path}",{info["cluster_id"]},{info["pseudo_label"]},"{info["pseudo_label_name"]}",{info["true_label"]},"{info["true_label_name"]}",{"yes" if info["correct"] else "no"}\n')
...
# Load simple mapping
import json
mapping = json.load(open('image_path_to_pseudolabel_simple.json'))
pseudo_label = mapping['train/0000/train_0000000.JPEG']  # For training image

# Or use CSV
import pandas as pd
df = pd.read_csv('train_image_pseudo_labels.csv')
pseudo_label = df[df['image_path'] == 'train/0000/train_0000000.JPEG']['pseudo_label_class_index'].values[0]
""")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Create ImageNet-1K pseudo-label mappings')
    parser.add_argument('--paths_file', type=str,
                        default='results/dinov2_large_imagenet1k_with_paths/image_paths.json',
                        help='Path to image_paths.json file')
    parser.add_argument('--pseudo_labels_dir', type=str,
                        default='results/imagenet1k_pseudolabels/pseudo_labels',
                        help='Directory containing pseudo labels')
    parser.add_argument('--predictions_file', type=str,
                        default='results/imagenet1k_pseudolabels/predictions.npz',
